...
// ============================================================================
// CruxEngine PAS — Procedural Asset Synthesis
// ============================================================================
    ...
    void generateRock(f32 radius, u32 subdivisions, f32 erosionStrength,
        ...
        for (auto& tri : tris) {
            for (Vec3* v : {&tri.a, &tri.b, &tri.c}) {
                ...
                f32 ny = v->y * 2.0f;
                f32 nz = v->z * 2.0f;
                f32 erosion = noise_.fbm2D(nx * 3.0f, nz * 3.0f, 4) * erosionStrength;
    ...
    void generateBranch(ProceduralMesh& out, Vec3 start, Vec3 dir,
        ...
        for (u32 i = 0; i <= segments; i++) {
            ...
            Vec3 right = (std::abs(dir.y) > 0.99f)
                ? Vec3(1, 0, 0) : Vec3::cross(dir, Vec3(0, 1, 0)).normalized();
            Vec3 up = Vec3::cross(right, dir).normalized();
        ...
        for (u32 i = 0; i < childCount; i++) {
            f32 angle = (f32)i / (f32)childCount * 6.2831853f + branchNoise.perlin2D((f32)seed, (f32)i) * 0.5f;
            f32 tilt = 0.3f + branchNoise.perlin2D((f32)i, (f32)seed * 0.1f) * 0.3f;
