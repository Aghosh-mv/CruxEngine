...
// ============================================================================
// CruxEngine Compute Shader Manager
// ============================================================================
...
public:
    ...
    bool init() {
        ready_ = true;
        CRUX_LOG_INFO("[ComputeManager] initialized");
        return true;
    }
    ...
    u32 createCompute(const char* source, const char* name = "compute",
        ...
        if (!ok) {
            ...
            CRUX_LOG_ERROR("[ComputeManager] shader '%s' compile error: %s", name, log);
            Gl::DeleteShader(cs);
            return 0xFFFFFFFF;
        }
        ...
        if (!ok) {
            ...
            CRUX_LOG_ERROR("[ComputeManager] shader '%s' link error: %s", name, log);
        ...
        CRUX_LOG_INFO("[ComputeManager] compute shader '%s' created (workgroup %u,%u,%u)",
                       name, wgX, wgY, wgZ);
        return idx;
    }
