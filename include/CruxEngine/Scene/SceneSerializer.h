...
// ============================================================================
// CruxEngine Scene Serializer — Save/load scenes to binary
// ============================================================================
...
public:
    bool saveBinary(const char* path, const Scene& scene,
                    ...
                    const char* author = "CruxEngine",
                    const char* description = "") {
        FILE* f = fopen(path, "wb");
        if (!f) {
            CRUX_LOG_ERROR("[SceneSerializer] cannot open %s for writing", path);
            return false;
        }

        // Collect all nodes
        Vector<SceneNodeID> nodes;
        scene.flatten(nodes);
        ...
        fclose(f);
        CRUX_LOG_INFO("[SceneSerializer] saved %u nodes to %s", header.nodeCount, path);
        return true;
    }
    ...
    bool loadBinary(const char* path, Scene& scene, Vector<u32>& entityIDs) {
        ...
        if (!f) {
            CRUX_LOG_ERROR("[SceneSerializer] cannot open %s", path);
            return false;
        }
        ...
        if (header.magic != SCENE_MAGIC) {
            CRUX_LOG_ERROR("[SceneSerializer] invalid magic in %s", path);
            fclose(f);
            return false;
        }
        if (header.version != SCENE_VERSION) {
            CRUX_LOG_ERROR("[SceneSerializer] version mismatch in %s", path);
            fclose(f);
            return false;
        }
        ...
        for (u32 i = 0; i < header.nodeCount; i++) {
            ...
            SceneNodeID newID = scene.createNode(parent);
        ...
        fclose(f);
        CRUX_LOG_INFO("[SceneSerializer] loaded %u nodes from %s", header.nodeCount, path);
        return true;
    }
    ...
    bool saveJSON(const char* path, const Scene& scene) {
        ...
        fclose(f);
        CRUX_LOG_INFO("[SceneSerializer] exported JSON to %s", path);
        return true;
    }
};
