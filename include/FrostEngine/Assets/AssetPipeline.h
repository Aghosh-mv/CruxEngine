...
// ============================================================================
// FrostEngine Asset Pipeline — Import, load, cache, hot-reload
// ============================================================================
...
public:
    bool load(const char* path, Vector<OBJMeshData>& outMeshes) {
        ...
        fclose(f);
        FROST_LOG_INFO("[OBJImporter] loaded %s (%u meshes)", path, (u32)outMeshes.size());
        return true;
    }
};
    ...
    u32 checkHotReload() {
        ...
        for (u32 i = 0; i < assetCount_; i++) {
            ...
            if (mod > info.lastModified) {
                ...
                info.dirty = true;
                reloaded++;
                FROST_LOG_INFO("[AssetManager] hot-reload: %s", info.path.data());
            }
        }
        return reloaded;
    }
