...
public:
    ...
    void insert(usize pos, char c) {
        ...
        memmove(data_ + pos + 1, data_ + pos, size_ - pos + 1);
        data_[pos] = c;
        size_ = newSize;
    }

    void erase(usize pos, usize len = 1) {
        if(pos >= size_) return;
        if(pos + len > size_) len = size_ - pos;
        memmove(data_ + pos, data_ + pos + len, size_ - pos - len + 1);
        size_ -= len;
    }
