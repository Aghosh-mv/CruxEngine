...
// ============================================================================
// FrostEngine Editor — Full editor framework (viewport, panels, inspector)
// ============================================================================
// Provides:
...
struct ViewportCamera {
    ...
    void pan(f32 dx, f32 dy) {
        ...
        Vec3 u = Vec3(0, 1, 0);
        target = target + right * dx * moveSpeed * 0.01f + u * dy * moveSpeed * 0.01f;
    }
    ...
    Vec3 forward() const {
        if (mode == Orbit) return (target - cameraPosition()).normalized();
    ...
    Mat4 viewMatrix() const {
        return Mat4::lookAt(cameraPosition(), target, up);
    }
    ...
    void moveForward(f32 dt) {
        ...
        if (mode != Orbit) position = position + fwd * moveSpeed * dt;
    }
    ...
    void moveLeft(f32 dt) {
        Vec3 r = right();
        target = target - r * moveSpeed * dt;
        if (mode != Orbit) position = position - r * moveSpeed * dt;
    }
    ...
    void moveUp(f32 dt) {
        target = target + up * moveSpeed * dt;
        if (mode != Orbit) position = position + up * moveSpeed * dt;
    }
